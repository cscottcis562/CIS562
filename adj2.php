...
$query_rsGames = "SELECT * FROM CFGames ORDER BY DivisionName, CFGames.HomeTeamName";
$rsGames = mysql_query($query_rsGames, $AYSO) or die(mysql_error());
$row_rsGames = mysql_fetch_assoc($rsGames);
$totalRows_rsGames = mysql_num_rows($rsGames);
?><!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<body>
Choose the team for Adjustment
<form action="AdjustmentsSchedule.php" method="post" name="TeamSelect">
  <p>
...
TCN_makeComboGroup("Division","TeamName","TeamID");
 var separator="+#+";
<?php do{?>
TCN_addContent("<?php echo $row_rsGames['DivisionName']; ?>+#+<?php echo $row_rsGames['DivID']; ?>+#+<?php echo $row_rsGames['HomeTeamName']; ?>+#+<?php echo $row_rsGames['HomeTeamID']; ?>+#+<?php echo $row_rsGames['HomeTeamID']; ?>+#+<?php echo $row_rsGames['HomeTeamID']; ?>");
<?php } while ($row_rsGames = mysql_fetch_assoc($rsGames)); ?>
TCN_reload();

    </script>
  </p>
  <p>
    <input name="SubmitTeamSelect" type="submit" id="SubmitTeamSelect" value="Submit">
</p>
</form>
</body>
